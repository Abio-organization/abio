...
export function UsernameStep() {
  ...
  const isOwnUsername =
    trimmed.length > 0 && trimmed === user?.profile?.username;
  ...
  const canSubmit =
    trimmed.length >= 3 &&
    !shapeError &&
    (isOwnUsername || isAvailable === true);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateProfileMutation.mutate(
      { username: trimmed },
      {
        onSuccess: () => navigate({ to: "/onboarding/themes" }),
        onError: (error) =>
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-4 py-8 sm:px-6">
        <div className="w-full max-w-md">
          <div className="mb-8 text-center">
            <h1 className="text-xl font-bold text-[#331400] md:text-2xl dark:text-[#F5EEE4]">
              Claim your free username
            </h1>
            <p className="mt-2 text-sm text-[#666464] dark:text-[#F5EEE4]/60">
              Choose a unique username that represents you.
            </p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-3">
            <div className="relative flex h-12 items-center border border-[#331400]/20 bg-transparent focus-within:border-[#331400] dark:border-[#F5EEE4]/20 dark:focus-within:border-[#F5EEE4]">
              <span className="pl-3 text-sm font-semibold text-[#331400]/60 select-none dark:text-[#F5EEE4]/60">
                abio.site/
              </span>
              <input
                value={username}
