...
export function LinksStep() {
  ...
  const handleSubmit = async () => {
    ...
    const customLinksToSave = customLinks
      ...
      .map((link) => ({
        title: "Custom Link",
        url: normalizeCustomUrl(link.url),
        platform: "custom",
    ...
    if (failedCount > 0) {
      toast.warning(`${failedCount} link(s) failed to save`, {
        description: `Saved ${results.length - failedCount} of ${results.length}.`,
      });
    } else {
      toast.success("Links saved", {
        description: `Added ${results.length} link(s).`,
      });
    }
  ...
  return (
    <OnboardingLayout step={5}>
      <div className="flex flex-1 justify-center px-4 py-4 sm:px-6">
        <div className="w-full max-w-md">
          <div className="mb-6 text-center">
            ...
            <p className="text-sm text-[#666464] dark:text-[#F5EEE4]/60">
              Complete the field below to add<br className="md:hidden"/>your content on you new Abio.
            </p>
          </div>
          ...
          <div className="space-y-5">
            {selectedPlatforms.length > 0 && (
              <div className="space-y-3">
                <h2 className="text-center text-sm font-semibold text-[#331400] dark:text-[#F5EEE4]">
                 Your Selected platforms
                </h2>
                {selectedPlatforms.map((platform) => (
                  <div key={platform.id} className="flex items-center gap-3">
                    <img
                      src={platform.linksIcon ?? platform.icon}
                      alt=""
            ...
            {/* <div className="space-y-3">
              ...
              {customLinks.map((link) => (
                <CustomLinkRow
                  ...
                  onUrlChange={(url) =>
                    setCustomLinks((links) =>
                      links.map((l) => (l.id === link.id ? { ...l, url } : l)),
                    )
                  }
                  onRemove={
