...
import { GOALS } from "@/features/onboarding/data";
...
export function GoalStep() {
  ...
  const handleSubmit = () => {
    ...
    updateProfileMutation.mutate(
      { goals: [goal.title] },
      {
        onSuccess: () => navigate({ to: "/onboarding/username" }),
        onError: (error) =>
                ...
                >
                  <div
                    className={cn(
                      ...
                      isSelected
                        ? "bg-[#FED45C]"
                        : "bg-[#331400]/5 dark:bg-white/10",
                    )}
                  >
                    <Icon
                      size={20}
                      className="text-[#331400] dark:text-[#F5EEE4]"
                    />
                  </div>
                  <div className="flex-1">
                    <h3 className="text-base font-bold text-[#331400] dark:text-[#F5EEE4]">
                      {goal.title}
                    </h3>
                    <p className="mt-1 text-xs text-[#666464] md:text-sm dark:text-[#F5EEE4]/60">
                      {goal.description}
                    </p>
                  </div>
                  {isSelected && (
                    <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#331400] dark:bg-[#F5EEE4]">
                      <Check
                        className="h-4 w-4 text-white dark:text-[#331400]"
                        strokeWidth={3}
                      />
                    </div>
                  )}
