...
export function AppearancePage() {
  ...
  const setPendingWallpaperFile = useAppearanceEditorStore(
    (s) => s.setPendingWallpaperFile,
  );
  ...
  useBlocker({
    shouldBlockFn: () => {
      ...
      return !window.confirm(
        "You have unsaved appearance changes. Leave without saving?",
      );
    },
    ...
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-3 text-center">
        <p className="text-sm text-red-600">
          Could not load your appearance settings.
        </p>
        <button
          type="button"
          ...
          {TABS.map((t) => (
            <button
              ...
              className={cn(
                ...
                tab === t.id
                  ? "text-[#331400] dark:text-[#F5EEE4]"
                  : "text-[#666464] dark:text-[#F5EEE4]/50",
              )}
            >
              {t.label}
              {tab === t.id && (
                <span className="absolute right-0 bottom-0 left-0 h-0.5 bg-red-500" />
              )}
            </button>
          ))}
        </div>
      ...
      <aside className="flex justify-center order-2 lg:order-1  lg:sticky lg:top-8 lg:h-fit">
        {previewLoading ? (
          ...
          <PhoneDisplay
            buttonStyle={current.buttonStyle}
            fontStyle={current.fontStyle}
            selectedTheme={current.wallpaper}
            profile={profile}
            links={links}
          />
        )}
